...
class scraper():
    def __init__(self, PROJECT_ID, DATASET, TABLE_NAME, SERVICE_ACCOUNT):
        ...
        self.url = "https://9to5google.com/wp-json/wp/v2/posts?"
    ...
    def transform_data(self, data: pd.DataFrame) -> pd.DataFrame:
        ...
        data["post_content"] = data["post_content"].apply(clean_html).apply(convert_to_utf8)
        ...
        while page_num <= 5:
            ...
            try:
                data = self.transform_data(
                    self.extract_data(self.make_request(self.get_url(self.url, page=page_num, **self.params)), self.cols)
                )
                # query data from DWH
                QUERY = f"""
